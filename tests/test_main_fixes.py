...
class TestBulkExport:
    ...
    @patch('src.main.os.path.isfile')
    @patch('src.main.os.access')
    def test_bulk_export_overrides_date_range_limit(self, mock_access, mock_isfile, mock_exists, mock_create_dir, mock_getenv, mock_load_json, mock_drive_client, mock_slack_client):
        ...
        with patch('src.main.get_conversation_display_name', return_value="test"):
            with patch('src.main.validate_channel_id', return_value=True):
                with patch('src.main.logger') as mock_logger:
                    ...
                    bulk_export_logged = any('bulk export' in str(call).lower() for call in log_calls)
    ...
    @patch('src.main.os.path.isfile')
    @patch('src.main.os.access')
    def test_bulk_export_chunks_large_exports(self, mock_access, mock_isfile, mock_exists, mock_create_dir, mock_getenv, mock_load_json, mock_drive_client, mock_slack_client):
            ...
            with patch('src.main.validate_channel_id', return_value=True):
                with patch('src.main.preprocess_history', return_value="test content"):
                    with patch('builtins.open', create=True) as mock_open:
                        with patch('src.main.os.path.getsize', return_value=100):
                            with patch('src.main.should_chunk_export', return_value=True) as mock_should_chunk:
                                with patch('src.main.split_messages_by_month') as mock_split:
                                    # Mock split to return chunks
                                    from datetime import datetime, timezone
                                    chunk1 = (datetime(2023, 1, 1, tzinfo=timezone.utc), 
                                             datetime(2023, 1, 31, 23, 59, 59, tzinfo=timezone.utc),
                                             history[:2])
                                    chunk2 = (datetime(2023, 2, 1, tzinfo=timezone.utc),
                                             datetime(2023, 2, 28, 23, 59, 59, tzinfo=timezone.utc),
                                             history[2:4])
                                    chunk3 = (datetime(2023, 3, 1, tzinfo=timezone.utc),
    ...
    @patch('src.main.os.path.isfile')
    @patch('src.main.os.access')
    def test_bulk_export_overrides_message_limit(self, mock_access, mock_isfile, mock_exists, mock_create_dir, mock_getenv, mock_load_json, mock_drive_client, mock_slack_client):
        ...
        large_history = [{"ts": "1234567890.123", "user": "U123", "text": "Message"} for _ in range(60000)]
    ...
    @patch('src.main.os.path.isfile')
    @patch('src.main.os.access')
    def test_bulk_export_file_size_warning(self, mock_access, mock_isfile, mock_exists, mock_create_dir, mock_getenv, mock_load_json, mock_drive_client, mock_slack_client):
        ...
        mock_slack_instance = Mock()
        mock_slack_instance.fetch_channel_history.return_value = [{"ts": "1234567890.123", "user": "U123", "text": "Message"}]
        mock_slack_instance.get_channel_members.return_value = []
        mock_slack_instance.get_user_info.return_value = {"displayName": "Test User"}
        mock_slack_client.return_value = mock_slack_instance
        ...
        with patch('src.main.get_conversation_display_name', return_value="test"):
            with patch('src.main.validate_channel_id', return_value=True):
                with patch('src.main.preprocess_history', return_value="x" * 200 * 1024 * 1024):  # 200MB content
                    with patch('builtins.open', create=True):
                        with patch('src.main.os.path.getsize', return_value=200 * 1024 * 1024):  # 200MB file
                            with patch('src.main.logger') as mock_logger:
                                main(args)
                                # Verify warning was logged for large file size
                                warning_calls = [str(call) for call in mock_logger.warning.call_args_list]
                                size_warning = any('exceeds maximum' in str(call).lower() or 'file size' in str(call).lower() for call in warning_calls)
